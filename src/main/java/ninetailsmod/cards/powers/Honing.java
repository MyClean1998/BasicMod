...
import ninetailsmod.powers.HoningPower;
import ninetailsmod.util.CardStats;

public class Honing extends BaseCard {

    public static final String ID = makeID(Honing.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public Honing() {
        super(ID, info);
        setMagic(ADDITIONAL_DAMAGE, UPG_DAMAGE);
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new HoningPower(p, this.magicNumber), this.magicNumber));
    }
}
