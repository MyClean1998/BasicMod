...
import ninetailsmod.actions.DiscardAllAttackAction;
import ninetailsmod.cards.BaseCard;
import ninetailsmod.characters.NineTailsCharacter;
import ninetailsmod.util.CardStats;

public class Stretch extends BaseCard {

    public static final String ID = makeID(Stretch.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    private static final int CARD_DRAWN = 2;

    public Stretch() {
        super(ID, info); //Pass the required information to the BaseCard constructor.
        setMagic(CARD_DRAWN, 1);
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new DiscardAllAttackAction());
